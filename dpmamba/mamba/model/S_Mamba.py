...
class Model(nn.Module):
    ...
    def process(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        print('x_enc, x_mark_enc',x_enc.shape)#4, 2, 640])
        ...
        print('enc_out1', enc_out.shape) #([8, 2, 512])
        ...
        enc_out, attns = self.encoder(enc_out, attn_mask=None)#enc_out
        print('enc_out2', enc_out.shape)#4, 640, 128
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        enc_out = self.process(x_enc, x_mark_enc, x_dec, x_mark_dec)
        print('mamba enc_out', enc_out.shape)

        #print('dec_out[:, -self.pred_len:, :]',dec_out[:, -self.pred_len:, :].shape)#[16, 96, 321])
        return enc_out#dec_out[:, -self.pred_len:, :]  # [B, L, D]