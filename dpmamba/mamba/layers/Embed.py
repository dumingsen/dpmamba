...
class DataEmbedding_inverted(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='h', dropout=0.1):
        ...
        self.value_embedding = nn.Linear(c_in, d_model)#c_in
        self.dropout = nn.Dropout(p=dropout)

    def forward(self, x, x_mark):
        print('DataEmbedding_inverted',x.shape)#[8, 45, 2])
        x = x.permute(0, 2, 1)

        # x: [Batch Variate Time]
        if x_mark is None:
            print('x_mark is None', x.shape)#[128, 3, 205]  x_mark is None torch.Size([4, 640, 2])
            x = self.value_embedding(x.float() )
            print('x', x.shape)#(([8, 2, 512])
        else:
